...
void valid_value_check(string value)
{
  const bool is_valid=is_string_numerical_value(value);

  if(not is_valid)
    {
      throw std::invalid_argument("Specific value "+value+" is invalid.");
    }
}
...
void read_equation(const string line)
{
  ...
  const vector<string> equation_read_result=equation_string_to_equation_vector(equation_value);
  equations_map[equation_name]=equation_change_order(equation_read_result);
}

void are_bin_params_defined(const bool param_found, const string param_name)
{
   if(param_name==EQUATIONS)
    {
      if(not param_found)
	{
...
void read_bin(const string directory)
{
		  ...
		  if(constants_spec_found)
		    {
		      ...
		      valid_value_check(initial_value);
  ...
  are_bin_params_defined(equations_found, EQUATIONS);
}
