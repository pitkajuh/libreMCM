...
vector<string> test2(vector<string> equation, unsigned int &open, unsigned int &close, unsigned int &k, MathOperations &ooo, const Data &data, MathOperation *e, MathOperation *next)
{
  bool end=false;
  vector<string> tmp;
  unsigned int open2;
  ...
  while(!end)
      ...
      if(open2<tmp.size())
	{
	  ...
	  equation=GetParenthesis(equation, open, close, k, ooo, data, e, next);
  ...
  while(!end)
    {
      ...
      if(is) equation=GetParenthesis(equation, open, close, k, ooo, data, e, next);
      else equation=test2(equation, open, close, k, ooo, data, e, next);
    }
  return equation;
}
